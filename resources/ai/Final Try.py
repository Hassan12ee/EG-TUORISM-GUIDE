...
X_places = places_df[['Type_encoded', 'Region_encoded', 'Rating', 'Popularity', 'Budget_encoded', 'Activity_Level_encoded', 'Indoor_Outdoor_encoded']]
...
place_model.fit(X_train_places, y_train_places)

X_restaurants = restaurants_df[['Food_encoded', 'Region_encoded', 'Rating', 'Popularity', 'Price_Range_encoded', 'Ambiance_encoded']]
...
restaurant_model.fit(X_train_restaurants, y_train_restaurants)

X_hotels = hotels_df[['Type_encoded', 'Region_encoded', 'Rating', 'Popularity_encoded', 'Budget_encoded', 'Facilities_encoded']]
    ...
    def do_POST(self):
        if self.path == '/get_recommendations':
            ...
            if not filtered_places_df.empty:
                input_place_df = pd.DataFrame([[type_place_encoded, region_encoded, filtered_places_df['Rating'].mean(), filtered_places_df['Popularity'].mean(), filtered_places_df['Budget_encoded'].mean(), filtered_places_df['Activity_Level_encoded'].mean(), filtered_places_df['Indoor_Outdoor_encoded'].mean()]], columns=['Type_encoded', 'Region_encoded', 'Rating', 'Popularity', 'Budget_encoded', 'Activity_Level_encoded', 'Indoor_Outdoor_encoded'])
                probas = place_model.predict_proba(input_place_df)
            ...
            if not filtered_restaurants_df.empty:
                input_restaurant_df = pd.DataFrame([[type_food_encoded, region_encoded, filtered_restaurants_df['Rating'].mean(), filtered_restaurants_df['Popularity'].mean(), filtered_restaurants_df['Price_Range_encoded'].mean(), filtered_restaurants_df['Ambiance_encoded'].mean()]], columns=['Food_encoded', 'Region_encoded', 'Rating', 'Popularity', 'Price_Range_encoded', 'Ambiance_encoded'])
                probas = restaurant_model.predict_proba(input_restaurant_df)
                top_indices = np.argsort(probas[0])[-n_recommendations:][::-1]
            ...
            if not filtered_hotels_df.empty:
                input_hotel_df = pd.DataFrame([[filtered_hotels_df['Type_encoded'].mean(), region_encoded, filtered_hotels_df['Rating'].mean(), filtered_hotels_df['Popularity_encoded'].mean(), filtered_hotels_df['Budget_encoded'].mean(), filtered_hotels_df['Facilities_encoded'].mean()]], columns=['Type_encoded', 'Region_encoded', 'Rating', 'Popularity_encoded', 'Budget_encoded', 'Facilities_encoded'])
                probas = hotel_model.predict_proba(input_hotel_df)
                top_indices = np.argsort(probas[0])[-n_recommendations:][::-1]
